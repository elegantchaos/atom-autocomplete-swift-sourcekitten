...
class SwiftProvider {
  ...
  getSuggestions(options) {
    ...
    if (prefix == "") {
      let t = editor.getBuffer().getTextInRange([[bufferPosition.row, bufferPosition.column - 2], [bufferPosition.row, bufferPosition.column]])
      if (t == "()") {
        offset -= 2
        adjustedPrefix = "("
      }
    ...
    console.debug("prefix: <" + prefix + ">", adjustedPrefix)
    console.debug("position:" + bufferPosition)
    console.debug('index: '+ index + ', offset: ' + offset)

    return this.findMatchingSuggestions(text, offset, adjustedPrefix);
  }

    findMatchingSuggestions(text, offset, prefix) {
      // call SourceKitten asynchronously
      return new Promise((resolve) => {
        const command = atom.config.get('atom-autocomplete-swift-sourcekitten.sourceKittenLocation') || "sourcekitten"
        ...
        let exit = function(code) {
          if (code == 0) {
            ...
            atom.notifications.addError("SourceKitten returned an error.", { description: "SourceKitten failed to get suggestions.\n\nError: ${code}." } )
            resolve([]);
          }
        }
        ...
        process.onWillThrowError((error) => {
          atom.notifications.addFatalError("Couldn't launch SourceKitten.", {
